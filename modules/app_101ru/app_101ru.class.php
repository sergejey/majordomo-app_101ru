...
//  DebMes("101msg");
    ...
    function getStreamURL($id)
    {
        ...
        DebMes('Getting radio page from ' . $rec['PAGE_URL'], '101ru');
        if ($rec['PAGE_URL']) {
            ...
            if (preg_match('/(\/api\/channel\/getListServersChannel\/.+?)[\'"]/isu', $data, $matches)) {
                ...
                $playlist_url = $radio_data['result']['playlist'][0]['file'];
                if ($playlist_url != '') {
                    return $playlist_url;
                } else {
                    DebMes("Cannot find playlist in " . $json_url, '101ru');
                }
            } elseif (preg_match('/id="footer-player" src="(http.+?)"/isu', $data, $matches)) {
                $playlist_url = $matches[1];
                return $playlist_url;
            } else {
                DebMes("Cannot find playlist in " . $rec['PAGE_URL'], '101ru');
            }
        }
        return false;
    }
